...
y= 0
for (i in 1:50) {
  if (x1[i]<=mean(x1[i]+x2[i])&&mean(x1[i]+x2[i])>mean(x2)) {
    y[i]=0
  }
  else{
    y[i]=1
...
#d
#Perbedaan regresi konvensional dengan bootstrap :
#Regresi bootstrap memiliki bias tertentu terhadap regresi konvensional,
#karena hasil dari regresi bootstrap ini merupakan hasil rata-rata dari pengulangan
#random hasil regresi konvensional.